...
function sync(){
 const p=by[config.pokemonId];el.search.value=monLabel(p);el.level.value=config.level;syncRange();el.levelOut.textContent="Lv."+config.level;el.nature.value=config.natureId;el.skill.value=config.mainSkillLevel;el.collect.value=config.collectionHours;el.fav.checked=config.favoriteBerry;el.team.value=config.teamHelpingBonus;el.target.value=config.ingredientTarget||"";fillSubs();fillIngredients();meta();save();schedule();
}
function read(){
...
function bind(){
 ...
 [el.level,el.nature,el.skill,el.i0,el.i30,el.i60,el.target,el.collect,el.fav,el.team].forEach(x=>x.addEventListener(x.type==="range"?"input":"change",()=>{read();syncRange();el.levelOut.textContent="Lv."+config.level;fillSubs();fillIngredients();save();schedule()}));
