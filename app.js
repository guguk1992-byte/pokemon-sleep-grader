...
const el={search:$("#pokemonSearch"),selected:$("#selectedPokemon"),list:$("#pokemonList"),meta:$("#pokemonMeta"),level:$("#level"),levelExact:$("#levelExact"),levelButtons:$$(".level-shortcuts button"),levelOut:$("#levelValue"),nature:$("#nature"),natureNote:$("#natureNote"),skill:$("#mainSkillLevel"),versatileField:$("#versatileField"),versatile:$("#versatileSkill"),subs:$$(".subskill-select"),i0:$("#ingredient0"),i30:$("#ingredient30"),i60:$("#ingredient60"),target:$("#ingredientTarget"),collect:$("#collectionHours"),fav:$("#favoriteBerry"),team:$("#teamHelpingBonus"),result:$("#resultContent"),status:$("#resultStatus"),source:$("#sourcePokemon"),share:$("#shareButton"),reset:$("#resetButton"),install:$("#installButton"),version:$("#dataVersion")};
...
function fillNatures(p){
 const available=E.hasNature(p);el.nature.innerHTML="";
 if(available)D.natures.forEach(n=>el.nature.add(new Option(natureLabel(n),n.id)));
 else el.nature.add(new Option("성격 없음","HARDY"));
 el.nature.disabled=!available;el.natureNote.hidden=available;el.nature.value=available?config.natureId:"HARDY";
}
function fillSubs(){
...
function sync(){
 const p=by[config.pokemonId];if(!E.hasNature(p))config.natureId="HARDY";el.search.value="";el.search.placeholder="현재 "+p.ko+" · 새 포켓몬 이름 입력";el.selected.textContent="현재: "+p.ko;el.level.value=config.level;el.levelExact.value=config.level;syncRange();el.levelOut.textContent="Lv."+config.level;fillNatures(p);el.skill.value=config.mainSkillLevel;el.versatile.value=config.versatileSkill;el.versatileField.hidden=p.skill!=="Versatile";el.collect.value=config.collectionHours;el.fav.checked=config.favoriteBerry;el.team.value=config.teamHelpingBonus;el.target.value=config.ingredientTarget||"";fillSubs();fillIngredients();meta();save();schedule();
}
...
function webMCP(){if(!navigator.modelContext?.registerTool)return;try{navigator.modelContext.registerTool({name:"pokemon_sleep_grade",description:"포켓몬 슬립 개체를 동일 포켓몬 조합과 비교해 역할별 백분위로 평가합니다.",inputSchema:{type:"object",properties:{pokemonId:{type:"string"},level:{type:"number",minimum:1,maximum:80},natureId:{type:"string",description:"성격이 있는 포켓몬에만 적용"},versatileSkill:{type:"string",description:"뮤의 올마이티로 배운 메인 스킬"},subskills:{type:"array",items:{type:"string"},minItems:5,maxItems:5}},required:["pokemonId","level","subskills"]},execute:async x=>{const r=E.analyze(sanitize({...E.defaultConfig(x.pokemonId),...x}));return{content:[{type:"text",text:JSON.stringify({pokemon:r.pokemon.ko,role:r.role.label,mainSkill:r.role.skillName,grade:r.current.grade,topPercent:+r.current.topPct.toFixed(2),verdict:r.verdict.text})}]}}})}catch{}}
function installPWA(){
